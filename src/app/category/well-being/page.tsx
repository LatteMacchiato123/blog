...
import { WellBeingHero } from "@/components/hero/WellBeingHero";

import type { Entry, EntrySkeletonType } from "contentful";
export const metadata = {
  title: 'Well-Being | Travelog',
  description: 'Well-Being Category of Travelog ',
};
...
export default async function WellBeing() {
  ...
  const wellBeingPosts = posts.filter((post) =>
    (post.fields.categories as Entry<EntrySkeletonType, undefined, string>[])
      .some((category) => category.fields.name === "Well-Being")
  );
  ...
  return (
    <div>
      ...
      <div className="max-w-screen-2xl mx-auto">
        {wellBeingPosts.length > 0 ? (
          <div className="pt-7">
            <h2 className="text-3xl font-bold text-gray-800 mb-4">
              Well-Being
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {wellBeingPosts.map((blog, idx) => (
                <div key={idx} className="rounded-lg">
                  <Link href={`/article/${blog.fields.slug}`} className="block">
                    <div className="relative w-full h-48 bg-gray-100">
                      <div className="w-full h-48 bg-gray-100">
                        <Image
                          src={`https:${(blog.fields.image as IContentfulAsset)?.fields.file.url}`}
          ...
          <div>
           ...
           Belum ada postingan dalam kategori<span className="italic"> Well-being</span>. Cek postingan terbaru kami di bawah ini:
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
